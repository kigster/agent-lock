...
`agent-lock` 0.1.0 is meant to let several agents, including several sub-agents of one session, work in one checkout without overwriting each other. Real-world testing on 2026-09-09 and 2026-09-11 showed it does not deliver that:
...
7. **Output misleads.** `list` counts interrupted (orphaned) locks as held and never flags stale ones; hints and error prefixes say `agent-lock` even when run as `alo`.
...
- **Messages name the program that was run** (`alo` or `agent-lock`).

## Out of scope

- `~/.agents/bin/agent-lock` (the old shell script) shadows the gem's `agent-lock` on PATH. That lives in the dot-agents repository.
- `alo help <command>` (dry-cli does not support it; `alo <command> --help` works).
